#version 120
// original SSAO shader graciously written at:
...
vec3 readNormal(vec2 coord) {
    return normalize(texture2D(normalMap, coord).xyz * 2.0  - 1.0);
}

vec3 posFromDepth(vec2 coord) {
    float d = texture2D(normalMap, coord).a;
...
void main(void) {
    ...
    vec3 random = texture2D(randomNoise, uv * fres.xy).rgb;
    ...
    float pw = incx;
    float ph = incy;
    float cdepth = texture2D(normalMap, uv).a;
    ...
    for(float i = 0.0; i < 3.0; ++i) {
        ...
        vec3 ddiff1 = posFromDepth(uv + vec2(npw, nph)) - p;
        ...
        gi +=  giFF(ddiff1, n, npw, nph) * texture2D(gdiffuse, uv + vec2(npw, nph)).rgb;
