...
void main(void) {
	vec2 coord = pos.xy / pos.w * 0.5 + 0.5;
	vec4 p = texture2D(positionMap, coord);			// get the position from deferred shading
	...
	vec3 H = normalize(VP + normalize(camPos - p.xyz));				// calculate half vector
